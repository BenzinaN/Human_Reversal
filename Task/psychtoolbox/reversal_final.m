...
DrawText(io.video.h,'Nous allons commencer la tâche...');
if DEBUG
    DrawText(io.video.h, { ...
        'DEBUG mode' '' ...
...
DrawText(io.video.h, { 'Vous devez choisir le symbole qui vous rapporte globalement le plus de points.' ...
            sprintf(' ') ...
            sprintf('Attention ! Le symbole qui vous rapporte le plus de point change au cours du temps !') ...
            sprintf('A vous donc de vous adapter pour gagner un maximum de points.') ...
            sprintf(' ') ...
            sprintf('Vous partez avec une base de 100 points.') ...
            sprintf('Une bonne réponse donne 1 point MAIS une mauvaise réponse retire 1 point !') ...
%             sprintf(' ') ...
%             sprintf('Par moment, vous devrez choisir à quel point vous êtes sûr de votre choix.') ...
%             sprintf('Ce choix se fera sur une échelle allant de 1 (pas du tout sûr) à 6 (sûr et certain).') ...
%             sprintf('Dans ces cas, le nombre de points en jeu dépend de votre degré de certitude.') ...
%             sprintf('Plus haute sera votre certitude,  plus important sera le nombre de points en jeu. Et vice-versa ;-)') ...
            })  
        Screen('Flip',io.video.h);
        
        fprintf('Appuyez sur [%s] pour lancer la tâche !\n',KbName(keywait));
        key = WaitKeyPress([keywait]);
...
while ~stopped && ~completed
    ...
    if i_trial_per_stims == 0
        ...
        DrawText(io.video.h,{'Ca va démarrer !'});
        Screen('Flip',io.video.h);
    ...
    if i_trial >= confidence_trials(end)
        confidence_trials(end+1) = i_trial+randpick(task.confidence_sampling(1):task.confidence_sampling(end));
        fprintf('Next confidence rating will be at trial %03d\n',confidence_trials(end));
    end
    ...
    prob_error = false;
    % Then we apply the criteria for reversal:
    % (crit. 1) we must let a certain number of trials after the reversal
    if i_trial_per_rev > task.no_error_after_rev
        %  (crit. 2) No more than N errors in the last N trials
        if sum(Data.prob_error(i_trial-1:-1:max(1,i_trial-task.max_errors_slidingwindow(2))))<=task.max_errors_slidingwindow(1)
            % (crit. 3) if we are already above the upper limit 
            % Otherwise not all previous consecutive trials with prob. errors
            if i_trial_per_rev < task.max_consecutive_error ||...
                    not(all(Data.prob_error(i_trial-1:-1:i_trial-task.max_consecutive_error)))
    ...
    % Sample confidence
    keyresp_conf=KbName({'1!','2@','3#','4$','5%','6^'});
    if i_trial == 0 ... replace by "confidence_trials(end)" if you want confidence scoring
        % Display confidence rating scale
        oldFont = Screen('TextFont',video.h);
        newFont = Screen('TextFont',video.h,'Courier New');
        Screen('TextStyle',video.h,1);
        DrawText(io.video.h,'1 2 3 4 5 6 ?','cmb',65);
        t=Screen('Flip',io.video.h,0,1);
        ...
        while resp_conf==0 && ~stopped
            % Look at response from keyboard
            if ~resp_conf && participant.flags.with_response_keyboard
                [resp_conf,resp_conf_t]=CheckKeyPress(keyresp_conf);
            end
            % Stop by experimenter ?
            if CheckKeyPress(keystop)
                t = NaN;
        ...
        Data.timecode.t_conf_resp(i_trial) = resp_conf_t;
        
        % Reaction time
        rt_conf = resp_conf_t-t;
        fprintf('\n|Confidence rating => choice made in %3.0fms ',1000*rt_conf);
        
        % Highlight the chosen stimulus

        if resp_conf == 1
            DrawText(io.video.h,'1            ','cmb');
        elseif resp_conf == 2
            DrawText(io.video.h,'  2          ','cmb');
        elseif resp_conf == 3
            DrawText(io.video.h,'    3        ','cmb');
        elseif resp_conf == 4
            DrawText(io.video.h,'      4      ','cmb');
        elseif resp_conf == 5
            DrawText(io.video.h,'        5    ','cmb');
        elseif resp_conf == 6
            DrawText(io.video.h,'          6  ','cmb');
        end
        
        t=Screen('Flip',io.video.h,0,1);
        fprintf('and was %d',resp_conf);
        newFont = oldFont;
        ...
        Data.resp_conf(i_trial) = resp_conf;
        Data.rt_conf(i_trial) = rt_conf;
        
    else
        Data.resp_conf(i_trial) = 0;
        Data.rt_conf(i_trial) = 0;
    end

    fprintf('\n|Score = %d (%d)',n_points,n_points-prev_n_points);
    ...
    if n_correct_in_a_row >= task.crit_rev
        ...
        fprintf('\n|%d >= %d trials were correct: ',n_correct_in_a_row,task.crit_rev);
        if prob_error
             fprintf('we don''t reverse on prob. errors.\n');
        elseif rand < task.prob_rev(1) || n_correct_in_a_row >= task.force_rev
    ...
    if i_rev > task.end_after_n_rev
        completed=true;
        fprintf('C''est fini...\n');
        fprintf('[Score final du sujet = %d]\n',n_points);
        DrawText(io.video.h, { 'FIN DE L''ETUDE'...
            sprintf('[Votre score final : %d]',n_points) ...
            })
        Screen('Flip',io.video.h);
        fprintf('Appuyez sur [%s] pour terminer !',KbName(keywait));
        key = WaitKeyPress([keywait]);
 
    elseif i_trial_per_rev==0 && i_rev>1 && mod(i_rev-1,task.pause_every_n_rev)==0
        fprintf('Pause...\n');
        DrawText(io.video.h, { 'On va faire une pause... ' ...
            sprintf('C''est vous qui décidez quand y retourner !') ...
            sprintf(' ') ...
            sprintf('(Progression : %d/100)',((i_rev-1)/task.end_after_n_rev)*100) ...
            sprintf(' ') ...
            sprintf('[Votre score : %d]',n_points) ...
            })        
        Screen('Flip',io.video.h);
        ...
        fprintf('Appuyez sur [%s] si patient ok pour y retourner !\n',KbName(keywait));
        key = WaitKeyPress([keywait]);
...
if DEBUG
    ...
    Screen('Preference', 'SkipSyncTests', 0); % mis à 1 pour portable Nabil ! Remettre à 0 pour les autres
end
if video.i<=1
    if DEBUG
