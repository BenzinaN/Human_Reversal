...
task.no_error_after_rev = 3; % number of trials free of prob. errors
task.max_consecutive_error = 3; % maximum number of consecutive probabilistic errors
task.max_errors_slidingwindow = [3 10]; % max. 3 errors over the last 10 trials
task.crit_rev = 6; % number of correct trials before rerv
task.prob_rev = 0.25; %[1-cumprod((1-.25)*ones(1,5)) 1]; 
task.pause_every_n_rev = 6 ;
task.end_after_n_rev = 24 ; % Number of reversals across whole session
task.force_rev = 15 ; % Force reversal after N correct trials

task.confidence_sampling = [3 10] % Sample confidence level every X trials
